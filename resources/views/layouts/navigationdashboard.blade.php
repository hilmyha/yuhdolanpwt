    ...
    <div class="flex flex-wrap justify-between items-center">
        ...
        <div class="flex items-center lg:order-2">
            @if (date('H') >= 0 && date('H') <= 11)
                <p class="text-sm text-gray-500">Selamat Pagi, {{ Auth::user()->email }}</p>
            @elseif (date('H') >= 12 && date('H') <= 14)
                <p class="text-sm text-gray-500">Selamat Siang, {{ Auth::user()->email }}</p>
            @elseif (date('H') >= 15 && date('H') <= 17)
                <p class="text-sm text-gray-500">Selamat Sore, {{ Auth::user()->email }}</p>
            @elseif (date('H') >= 18 && date('H') <= 23)
                <p class="text-sm text-gray-500">Selamat Malam, {{ Auth::user()->email }}</p>
            @endif
        </div>
    </div>
</nav>
